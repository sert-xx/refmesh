...
`register` は新規 Concept の embedding を既存ベクトルインデックスと照合し、類似度 ≥ 0.95 の既存ノードがあれば
...
- **Overview**: Concept / Reference / Edge 件数、Edge type 別の分布、SQLite DB のパス・サイズ・ベクトル行数。
- **Concepts**: 一覧 + ページング + ソート (lastSeenAt / touchCount / id)。`archived` も任意で表示。
- **Search**: 自然言語クエリで `refmesh search` 相当のスコアリング検索を実行。
- **Search Debug**: 検索パイプラインを段階別に可視化。クエリ埋め込みの形状 (dim / L2ノルム / 全次元プレビュー)、ベクトルインデックスへの `oversample` と `threshold` 前の全ベクトルヒット (棄却分含む)、グラフ探索で発行された SQL クエリ一覧、各候補のスコア内訳 (cosine / freshness / reinforcement / lexical / bm25 / final と除外理由)、近傍展開の各 level のフロンティア・追加 edge 数を表示する。`accessCount` を更新しない読み取り専用。
- **Graph**: 起点 Concept を指定するとグラフを描画。**ノードクリックで近傍を増分展開**でき、Edge type ごとに色分けされる。

`Ctrl+C` で停止し、SQLite ハンドル（statement cache + db）をクリーンに閉じる。
...
`prune` は `lastSeenAt` が cutoff より古く `touchCount <= --max-touches` のノードを Concept とベクトルインデックスの両方から削除する。
...
│   ├── register.ts        # refmesh register (Concept + Vector + FTS の同期更新)
│   ├── search.ts          # refmesh search (cosine + freshness + reinforcement + lexical + bm25 の合成スコア)
...
├── console/
│   ├── handlers.ts        # 読み取り専用 API (stats / concepts / neighbors / search / search-debug)
│   ├── server.ts          # loopback 限定の HTTP サーバ + 静的アセット配信
│   └── index.ts
├── db/
│   ├── store.ts           # SQLite 接続層 (better-sqlite3 + statement cache + WAL)
│   ├── migrations.ts      # スキーマ DDL とマイグレーション
│   ├── concept-repo.ts    # Concept / Reference / Edge の CRUD
│   ├── graph.ts           # edges テーブルの BFS / フロンティア展開
│   ├── fts.ts             # FTS5 仮想テーブルの構築・クエリ
│   ├── vector-index.ts    # インメモリ Float32Array ベクトルインデックス
│   ├── statement-cache.ts # prepared statement のキャッシュ
│   └── paths.ts           # DB パス解決 (~/.refmesh/refmesh.db, REFMESH_DB_PATH)
├── embedding/
